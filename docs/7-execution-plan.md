...
| v1.1.0 | 2026-04-02 | 모든 Task 완료 (2026-04-02) | - |
...
**완료 조건:**
- [x] `npm run dev` → `localhost:5173` 정상 렌더링
- [x] TypeScript 절대 경로 import 동작
...
**완료 조건:**
- [x] 3개 타입 파일 컴파일 오류 없음
- [x] User, Todo 속성이 도메인 정의서와 일치
- [x] TodoStatus 4종 열거형 정의
- [x] ApiResponse, ApiListResponse, ApiErrorResponse 제네릭 타입 정의
...
**완료 조건:**
- [x] Axios 인스턴스에 baseURL, 인증 헤더 인터셉터 설정
- [x] 401 응답 시 토큰 초기화 + 로그인 페이지 리다이렉트
- [x] API 함수 10개 (auth 3 + todo 7) 구현
- [x] 엔드포인트가 프로젝트 구조 문서와 일치
...
**완료 조건:**
- [x] `setAuth` 후 `isAuthenticated === true`
- [x] `clearAuth` 후 token/user null, `isAuthenticated === false`
- [x] localStorage/sessionStorage 미사용
- [x] API 인터셉터에서 `getToken()` 정상 참조
...
**완료 조건:**
- [x] `computeTodoStatus`가 4가지 상태 정확 산출
- [x] 비밀번호 검증이 PRD 정책 준수
- [x] 이메일 RFC 5322 형식 + 255자 검증
- [x] 종료일 < 시작일 시 오류 메시지 반환
...
**완료 조건:**
- [x] 로그인 성공 시 auth store 토큰 저장 + 목록 페이지 이동
- [x] CRUD mutation 성공 시 쿼리 캐시 자동 무효화
- [x] 필터/정렬/페이지 파라미터별 캐싱 분리
- [x] `QueryClientProvider` 앱 루트 설정
...
**완료 조건:**
- [x] 7개 공통 컴포넌트 구현 (Button, Input, TextArea, Modal, Pagination, Select, StatusBadge)
- [x] Header에 로그아웃 버튼 + 사용자 이름 표시
- [x] Modal 확인/취소 동작
- [x] 360px ~ 1280px 반응형 지원
...
**완료 조건:**
- [x] 미인증 사용자 → `/todos` 접근 시 `/login` 리다이렉트
- [x] 인증된 사용자 → `/login` 접근 시 `/todos` 리다이렉트
- [x] 루트(`/`) 인증 상태별 올바른 이동
- [x] 401 토큰 초기화 시 자동 로그인 리다이렉트
...
**완료 조건:**
- [x] 필드별 유효성 오류 메시지 표시
- [x] 이메일 중복 시 서버 오류 메시지 표시
- [x] 가입 성공 → 로그인 페이지 이동
- [x] 360px 모바일 환경 정상 표시
...
**완료 조건:**
- [x] 미입력 시 제출 차단
- [x] 잘못된 자격 증명 → 통합 오류 메시지 (계정 존재 여부 비노출)
- [x] 로그인 성공 → 토큰 메모리 저장 + 목록 페이지 이동
- [x] 회원가입 링크 동작
...
**완료 조건:**
- [x] 페이지네이션(20건/페이지)과 함께 목록 표시
- [x] 상태 필터 5종 + "종료된 할일" 필터 동작
- [x] 시작일/종료일 기준 오름차순/내림차순 정렬
- [x] 할일 등록 폼 유효성 검증 + 등록 성공
- [x] 완료 체크박스 토글 동작
- [x] 삭제 확인 다이얼로그 + 삭제 동작
- [x] 360px ~ 1280px 반응형 레이아웃
...
**완료 조건:**
- [x] 상세 정보 전체 표시 (제목, 설명, 날짜, 상태, 생성/수정 일시)
- [x] 수정 모드 전환 시 기존 값 프리필 + 저장 동작
- [x] 미존재 할일 → 404 안내 표시
- [x] 완료/완료 취소 토글 동작
- [x] 삭제 후 목록 페이지 이동
...
**완료 조건:**
- [x] 360px에서 가로 스크롤 없이 정상 표시
- [x] 1280px에서 여백/레이아웃 적절
- [x] 모바일 터치 타겟 44px 이상
